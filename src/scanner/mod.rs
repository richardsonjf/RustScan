...
mod tests {
    ...
    fn scanner_runs() {
        ...
        let scanner = Scanner::new(&addr, 1, 1_000, 100, Duration::from_millis(100), true);
    ...
    fn ipv6_scanner_runs() {
        ...
        let scanner = Scanner::new(&addr, 1, 1_000, 100, Duration::from_millis(100), false);
        block_on(scanner.run());
        // if the scan fails, it wouldn't be able to assert_eq! as it panicked!
        assert_eq!(1, 1);
    }
    ...
    fn quad_zero_scanner_runs() {
        ...
        let scanner = Scanner::new(&addr, 1, 1_000, 100, Duration::from_millis(500), true);
        block_on(scanner.run());
        assert_eq!(1, 1);
    }
    #[test]
    fn google_dns_runs() {
        ...
        let scanner = Scanner::new(&addr, 400, 445, 100, Duration::from_millis(1_500), true);
        block_on(scanner.run());
        assert_eq!(1, 1);
    }
    #[test]
